# Troubleshooting — IT-Stack JITSI
...
docker compose logs --tail=50 jitsi
...
1. Check logs: docker compose logs jitsi
2. Verify environment variables are set correctly
3. Check database connectivity: pg_isready -h lab-db1 -p 5432
4. Verify port is not already in use: ss -tlnp | grep 443
...
```bash
# Test connectivity
psql -h lab-db1 -U jitsi_user -d jitsi_db -c '\conninfo'

# Check pg_hba.conf allows connection from lab-app1
```

### Performance issues

1. Check resource usage: docker stats it-stack-jitsi
...
| Log | Path |
|-----|------|
| Application | docker compose logs jitsi |
